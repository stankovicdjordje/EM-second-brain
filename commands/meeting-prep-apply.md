...
5. Parse every checkbox line under the note's `## Follow-ups`, `## Challenges`, `## Growth`, and `## Achievements` headings. Only lines checked `- [x]` are in scope - leave every `- [ ]` line alone, and don't infer intent beyond what's literally checked. (It's valid for the master box to be checked while zero itemized boxes are checked - that just means "reviewed, nothing to raise this time.")
6. Compile the checked items into a **grouped, multi-level nested outline** styled after the owner's own personal meeting-agenda convention - his running per-person notes doc (a Google Doc titled something like "Notes - `<Person>` / `<Owner>` - Weekly"), which groups by theme (e.g. "Personal", "Project specific", "AI") as plain top-level bullets with nested sub-bullets underneath, 2-3 levels deep. Group the compiled output the same way, using the checklist's own review categories - **Follow-ups, Challenges, Growth, Achievements**, in that order - as the top-level theme bullets. Only include a category if at least one of its items was checked; drop categories with nothing checked entirely rather than leaving an empty header.
   - Each category is a plain top-level bullet (its bare name, e.g. "Follow-ups" - no bold, matching how the owner's own doc's theme bullets read as plain text).
   - Nest one bullet per checked item directly under its category, in priority order (highest first) carried over from the checklist, with the priority/recurrence tag itself dropped.
   - Phrase every item bullet as something the owner would actually say out loud - **a direct question addressed to the person ("you"), or a stated idea/suggestion** - never a meta-instruction describing the act of asking. Wrong: "Ask whether Nemo has confirmed the 80% -> 100% allocation timing yet." Right: "Has Nemo confirmed the 80% -> 100% allocation timing yet, and can we announce it to the team?" An idea reads the same way: not "Offer to help with X" but "I can help you with X once Y lands - let's find the time."
   - Nest a further third-level sub-bullet under an item only where there's a genuine follow-on question or supporting reference (a link, a specific number, a "by when") - matching how the owner's own doc nests a clarifying sub-question under a topic (e.g. "Current satisfaction with your work (1 to 5)?" -> "Can you name a few?"). Don't force a third level onto an item that's already a single, self-contained ask.
   ...
   - After the last category group, always add two fixed, unnested closing bullets - "Anything from your side?" then "Q/A" - matching how the owner's own doc always ends every entry with a flat, ungrouped "Q/A" bullet outside any theme.
...
7. Present the compiled outline directly in chat, in a fenced code block using plain `-` markdown bullets with 2-space indent per nesting level, so the owner can copy-paste it straight into his Google Doc under a new dated heading - this is the primary deliverable, needed before the meeting starts. Lead with a one-line header (outside the code block) naming who the meeting is with and when.
8. **Rewrite the note entirely** rather than appending to it - the checklist (priority tags, recurrence counts, checkboxes, the Review status section) was scratch scaffolding for the owner's selection step and is not wanted in the final result. The note that lands in `Done/` contains ONLY:
   ...
   - A `## Notes` heading, then the grouped, multi-level nested bullet outline from step 6 (category groups in order, each with their nested items, ending with "Anything from your side?" / "Q/A")
