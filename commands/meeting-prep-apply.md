...
6. Compile the checked items into a **bullet list of talking points and questions, grouped by area** - this is the actual output, not a restatement of the checklist:
   - Group the compiled bullets under the same area headings the checklist itself uses (`### Follow-ups`, `### Growth`, `### Achievements`, plus `### Open items` or any other custom heading the owner added by hand) - preserve whichever of these headings actually had a checked item under it, in that order, and omit any heading with nothing checked under it (don't print an empty "### Achievements" if nothing was checked there).
   - Within each area, keep the priority ordering from the checklist (highest priority first).
