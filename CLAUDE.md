...
This is the source repo for **obsidian-second-brain**, a Claude Code skill that turns any Obsidian vault into a living AI-first second brain. The skill ships 47 slash commands across 4 layers (vault management, thinking tools, research toolkit, scheduled agents).
...
- `commands/` - 47 slash command definitions, one `.md` per command. **This is the platform-neutral source.** Adapters compile it for each platform.
...
- `adapters/` - platform translation layer. `lib.sh` holds shared parsing helpers. `claude-code/`, `codex-cli/`, `gemini-cli/`, `opencode/`, `hermes/`, `pi/`, `agent-skills/`, `grok-bot/` each ship an `adapter.sh`.
